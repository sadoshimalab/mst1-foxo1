...
threads=n # number of available threads
...
do
	...
	computeMatrix reference-point --referencePoint TSS -b 2000 -a 2000 -R $project_dir/analysis/genomeRef/Mus_musculus.GRCm39.105.gtf.gz -S $bamCoverage_out/$sample_name"_sorted_filtered.bw" --skipZeros -o $computeMatrix_out/$sample_name"_sorted_filtered.gz" --binSize 5 -p $threads --outFileSortedRegions $computeMatrix_out/$sample_name"_sorted_filtered.bed"

	# visualize the results with deepTools
	plotProfile -m $computeMatrix_out/$sample_name"_sorted_filtered.gz" -out $computeMatrix_out/$sample_name"_sorted_filtered_line.png" --colors red --plotTitle " " --samplesLabel " " --refPointLabel "TSS" -T "$sample_name" -z " " --dpi 1200 -y " " --yMin 0 --yMax 1
	plotHeatmap -m $computeMatrix_out/$sample_name"_sorted_filtered.gz" -out $computeMatrix_out/$sample_name"_sorted_filtered_heatmap.png" --colorMap RdBu --whatToShow 'heatmap and colorbar' --dpi 1200 -y " " --zMin 0 --zMax 3
...
do
	...
	findMotifsGenome.pl $macs3_out/$sample_name"_summits.bed" $genome_homer $homer_out/$sample_name -size 200 -preparse
